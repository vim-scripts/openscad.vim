...
" Language:	OpenSCad
" Maintainer:	John Cooper ( john at choffee.co.uk )
" Last Change:	2011-12-01
"
" TODO
"   Build list of variables and functions and off them as completion
"   Included imported libraries


function! openscadcomplete#CompleteOpenSCAD(findstart, base)
...
let opensrbracket   = strridx(line, '[')
let closesqrbracket = strridx(line, ']')
let openbracket     = strridx(line, '(')
let closebracket    = strridx(line, ')')
let semicolon       = strridx(line, ';')
let opencomm        = strridx(line, '//')
let percent         = strridx(line, '%')
let hash            = strridx(line, '#')

if opensqrbracket > -1
	let borders[opensqrbracket] = "opensqrbracket"
endif
if closesqrbracket > -1
	let borders[closesqrbracket] = "closesqrbracket"
endif
if openbracket > -1
	let borders[openbracket] = "openbracket"
endif
if closebracket > -1
	let borders[closebracket] = "closebracket"
endif
if opencomm > -1
...
if percent > -1
	let borders[percent] = "percent"
endif
if hash > -1
	let borders[hash] = "hash"
endif


if len(borders) == 0 || borders[max(keys(borders))] =~ '^\%(opensqrbracket\|openbracket\|semicolon\|opencomm\)$'
	" Complete properties
